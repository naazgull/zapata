...
auto zpt::net::transport::unix_socket::make_reply(bool _with_allocator) const -> zpt::message {
...
auto zpt::net::transport::unix_socket::process_incoming_request(zpt::stream _stream) const
  -> zpt::message {
...
auto zpt::net::transport::unix_socket::process_incoming_reply(zpt::stream _stream) const
  -> zpt::message {
    expect(_stream->transport() == "unix", "Stream underlying transport isn't 'unix'");
    auto _message = zpt::allocate_message<zpt::json_message>();
    (*_stream) >> std::noskipws >> _message;
