...
zpt::Re2cFunctionalLexer::Re2cFunctionalLexer(std::istream& _in, std::ostream& _out)
  : __in{ &_in }
  , __out{ &_out } {
    this->resetBuffer();
}

zpt::Re2cFunctionalLexer::~Re2cFunctionalLexer() {}

auto zpt::Re2cFunctionalLexer::matched() const -> std::string const& { return this->__matched; }

auto zpt::Re2cFunctionalLexer::setMatched(std::string const& _text) -> void {
    this->__matched = _text;
}

auto zpt::Re2cFunctionalLexer::more() -> void { this->__more = true; }

auto zpt::Re2cFunctionalLexer::startCondition() const -> zpt::re2c_functional_cond {
    return this->__condition;
}

auto zpt::Re2cFunctionalLexer::begin(zpt::re2c_functional_cond _condition) -> void {
    this->__condition = _condition;
}

auto zpt::Re2cFunctionalLexer::lineNr() const -> std::size_t { return this->__line_nr; }

auto zpt::Re2cFunctionalLexer::resetBuffer() -> void {
...
auto zpt::Re2cFunctionalLexer::switchStreams(std::istream& _in, std::ostream& _out) -> void {
    this->__in = &_in;
    this->__out = &_out;
    this->__in->clear();
    this->resetBuffer();
    this->__condition = zpt::re2c_functional_cond::INITIAL;
...
auto zpt::Re2cFunctionalLexer::fill(std::size_t _need) -> bool {
...
auto zpt::Re2cFunctionalLexer::yyfill(std::size_t _need) -> void {
...
auto zpt::Re2cFunctionalLexer::captureMatch() -> void {
    this->__line_nr +=
...
auto zpt::Re2cFunctionalLexer::syncBackToStream() -> void {
...
auto zpt::Re2cFunctionalLexer::readRaw(std::size_t _n) -> std::string {
    std::string _out;
    _out.reserve(_n);
...
auto zpt::Re2cFunctionalLexer::leave(int _retValue) -> void {
    this->__left = true;
    this->__leave_value = _retValue;
}

auto zpt::Re2cFunctionalLexer::lex() -> int {
    while (true) {
        ...
        switch (this->__condition) {
            case zpt::re2c_functional_cond::INITIAL: _token = this->lexInitial(); break;
            case zpt::re2c_functional_cond::quoted: _token = this->lexQuoted(); break;
            case zpt::re2c_functional_cond::number: _token = this->lexNumber(); break;
        }
