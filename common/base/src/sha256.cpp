...
std::string zpt::crypto::sha256(std::string const& input) {
    ...
    char buf[2 * zpt::crypto::SHA256::DIGEST_SIZE + 1];
    buf[2 * zpt::crypto::SHA256::DIGEST_SIZE] = 0;
    for (size_t i = 0; i < zpt::crypto::SHA256::DIGEST_SIZE; i++) sprintf(buf + i * 2, "%02x", digest[i]);
