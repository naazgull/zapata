...
zpt::globals::alloc(ssize_t& _variable, Args... _args) -> T& {
    expect(_variable == -1, "variable already assigned with identifier " << _variable << "  for " << typeid(T).name());
...
zpt::globals::get(ssize_t _variable) -> T& {
    ...
    expect(_found != zpt::globals::__variables.end(), "no such global variable for " << typeid(T).name());
    expect(static_cast<ssize_t>(_found->second.size()) > _variable, "no such global variable for " << typeid(T).name());
...
zpt::thread_local_table::alloc(P const& _member_variable, Args... _args) -> T& {
    ...
    zpt::thread_local_table::__variables.emplace(reinterpret_cast<std::uintptr_t>(&_member_variable),
                                                 std::move(entry_type(_entry)));
    return **_entry;
}
...
zpt::thread_local_table::get(P const& _member_variable) -> T& {
    auto _found = zpt::thread_local_table::__variables.find(reinterpret_cast<std::uintptr_t>(&_member_variable));
