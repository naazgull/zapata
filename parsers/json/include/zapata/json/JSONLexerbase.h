...
class JSONLexerBase {
  ...
  protected:
    ...
    int const (*d_dfaBase_)[50];

    static int const s_dfa_[][50];
    static int const (*s_dfaBase_[])[50];
...
inline size_t JSONLexerBase::Input::lineNr() const { return d_lineNr; }
inline size_t JSONLexerBase::Input::nPending() const { return d_deque.size(); }
inline void JSONLexerBase::Input::setPending(size_t size) {
    d_deque.erase(d_deque.begin(), d_deque.end() - size);
}
inline void JSONLexerBase::Input::close() {
    delete d_in;
...
inline JSONLexerBase::StartCondition_ JSONLexerBase::startCondition() const {
