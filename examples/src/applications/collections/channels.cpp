...
zpt::ev::Patch,
zpt::path::join({ zpt::array, "v2", "datums", "applications", std::string(_tv_app_id), "users", std::string(_tv_user_id), "channels" }),
{ "headers", zpt::rest::authorization::headers(_identity["access_token"]), "payload", _envelope["payload"], "params", (_envelope["params"] + zpt::json({ "app_id", _tv_app_id, "user_id", _tv_user_id })) }
);
