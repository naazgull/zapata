...
auto zpt::apps::datums::Applications::get(std::string _topic,
...
auto zpt::apps::datums::Applications::query(std::string _topic,
					    zpt::json _filter,
					    zpt::ev::emitter _emitter,
					    zpt::json _identity,
					    zpt::json _envelope) -> zpt::json {
		...
		for (auto _d_element : _r_data["elements"]->arr()) {
			zpt::json _d_token = _emitter->route(
			    ...
			    {"headers",
			     zpt::rest::authorization::headers(_identity["access_token"]),
			     "params",
			     (_envelope["params"] + zpt::json({"id", std::string(_d_element["id"])}))})["payload"];
			_d_element << "token" << (_d_token["elements"]->type() == zpt::JSArray ? _d_token["elements"][0]
											       : _d_token);
		}
	}
...
auto zpt::apps::datums::Applications::insert(std::string _topic,
					     zpt::json _document,
					     zpt::ev::emitter _emitter,
					     zpt::json _identity,
					     zpt::json _envelope) -> zpt::json {
	...
	_r_data = {"href",
		   (_topic + std::string("/") + _c->insert("Applications", _topic, _document, {"href", _topic}))};
...
auto zpt::apps::datums::Applications::save(std::string _topic,
					   zpt::json _document,
					   zpt::ev::emitter _emitter,
					   zpt::json _identity,
					   zpt::json _envelope) -> zpt::json {
...
auto zpt::apps::datums::Applications::set(std::string _topic,
					  zpt::json _document,
					  zpt::ev::emitter _emitter,
					  zpt::json _identity,
					  zpt::json _envelope) -> zpt::json {
...
auto zpt::apps::datums::Applications::remove(std::string _topic,
					     zpt::ev::emitter _emitter,
					     zpt::json _identity,
					     zpt::json _envelope) -> zpt::json {
	zpt::json _r_data;
...
auto zpt::apps::datums::Applications::remove(std::string _topic,
	...
	_r_data = {
	    "href", _topic, "n_deleted", _c->remove("Applications", _filter, _filter + zpt::json({"href", _topic}))};
