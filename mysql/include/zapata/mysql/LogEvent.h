...
enum QueryVarType {
	...
	Q_CATALOG_CODE = 2,  // Value is the catalog name: a length byte followed by that many bytes, plus a terminating
			     // null byte. This variable is present only in MySQL 5.0.0 to 5.0.3. It was replaced with
			     // Q_CATALOG_NZ_CODE in MySQL 5.0.4 because the terminating null is unnecessary.
	Q_AUTO_INCREMENT = 3, // Value is two 2-byte unsigned integers representing the auto_increment_increment and
	// auto_increment_offset system variables. This variable is present only if auto_increment
	// is greater than 1.
	...
	Q_TIME_ZONE_CODE = 5, // Value is the time zone name: a length byte followed by that many bytes. This variable
	// is present only if the time zone string is non-empty.
	Q_CATALOG_NZ_CODE = 6, // Value is the catalog name: a length byte followed by that many bytes. Value is always
			       // std. This variable is present only if the catalog name is non-empty.
	Q_LC_TIME_NAMES_CODE = 7, // Value is a 2-byte unsigned integer representing the lc_time_names number. This
				  // variable is present only if the value is not 0 (that is, not en_US).
	Q_CHARSET_DATABASE_CODE =
	    8, // Value is a 2-byte unsigned integer representing the collation_database system variable.
	Q_TABLE_MAP_FOR_UPDATE_CODE = 9, // Value is 8 bytes representing the table map to be updated by a
...
typedef struct q_variables {
	...
	q_variables(q_variables const& _other) {
		...
		case zpt::mysql::Q_FLAGS2_CODE: { // Value is a 4-byte bit-field. This variable is written only as of
						  // MySQL 5.0.
		...
		case zpt::mysql::Q_CATALOG_CODE: { // Value is the catalog name: a length byte followed by that many
		...
		case zpt::mysql::Q_TIME_ZONE_CODE: { // Value is the time zone name: a length byte followed by that many
		...
		case zpt::mysql::Q_CATALOG_NZ_CODE: { // Value is the catalog name: a length byte followed by that many
		...
		case zpt::mysql::Q_CHARSET_DATABASE_CODE: { // Value is a 2-byte unsigned integer representing the
			// collation_database system variable.
			__charset_database_code = _other.__charset_database_code;
			break;
		}
		case zpt::mysql::Q_TABLE_MAP_FOR_UPDATE_CODE: { // Value is 8 bytes representing the table map to be
	...
	zpt::json to_json() {
		...
		case zpt::mysql::Q_CHARSET_CODE: {
			return {"character_set_client",
				std::get<0>(__charset_code),
				"collation_connection",
				std::get<1>(__charset_code),
				"collation_server",
				std::get<2>(__charset_code)};
		}
		case zpt::mysql::Q_TIME_ZONE_CODE: {
...
class LogEventPtr : public std::shared_ptr<zpt::mysql::LogEvent> {
	...
	static auto
	register_callback(zpt::mysql::LogEventType _event_type,
