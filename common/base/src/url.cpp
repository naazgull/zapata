...
auto
zpt::quoted_printable::encode(std::string const& _quote, std::string const& _charset, std::string& _out) -> void {
...
auto
zpt::quoted_printable::r_encode(std::string const& _quote, std::string const& _charset) -> std::string {
    std::string _out;
...
zpt::url::encode(std::string& _out) -> void {
    ...
    for (; pSrc < SRC_END; ++pSrc) {
        if ((*pSrc > 127) || (*pSrc == '%') || (*pSrc == ' ') || (*pSrc == '&') || (*pSrc == '+') || (*pSrc == '?') ||
            (*pSrc == '#') || (*pSrc == '=') || (*pSrc == '/') || (*pSrc == ':')) {
            *pEnd++ = '%';
            *pEnd++ = DEC2HEX[*pSrc >> 4];
