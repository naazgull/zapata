...
auto zpt::mysql::LogEventPtr::register_callback(zpt::mysql::LogEventType _event_type, std::function< zpt::mysql::event (zpt::mysql::event_header, std::istream& _in) > _consumer) -> void {
...
auto zpt::mysql::QueryEvent::consume(zpt::mysql::event_header _header, std::istream& _in) -> zpt::mysql::event {
	...
	if (_header->__event_length) {
			...
			switch(_var_type) {
				...
				case zpt::mysql::Q_UPDATED_DB_NAMES : {
					...
					for (uint8_t _arr_idx = 0; _arr_idx != _arr_length; _arr_idx++) {
						uint32_t _str_len = std::min< uint32_t >(NAME_LEN, _event->__variable_length - _idx);
				...
				default: {
					assertz(_var_type < 17, "deserialization error: query variable type is higher than 16", 0, 500);
				}
			}
		...
		size_t _query_length = _event->header()->__event_length - zpt::mysql::lengths::header - zpt::mysql::lengths::fixed::query_event - _event->__database_name_length - _event->__variable_length;
...
auto zpt::mysql::RotateEvent::consume(zpt::mysql::event_header _header, std::istream& _in) -> zpt::mysql::event {
	...
	if (_header->__event_length) {
		...
		_event->__next_binlog_filename = zpt::mysql::arr2str(_next_binlog_filename, _next_binlog_filename_size - 8);
	}
	return zpt::mysql::event(_event);
}
