...
auto to_pattern(std::string const& _path) -> zpt::json;
auto to_path(std::string const& _path) -> zpt::json;

} // namespace pipeline
} // namespace zpt

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::current() -> size_t {
    return this->__current;
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::marked() -> size_t {
    return this->__next;
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::at(size_t _requested_next) -> position& {
    this->__requested = _requested_next;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::first() -> position& {
    this->__requested = 0;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::last() -> position& {
    this->__requested = this->__max - 1;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::next() -> position& {
    this->__requested = this->__current + 1;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::position::mark() -> size_t {
...
template<typename T>
auto zpt::pipeline::event<T>::event_t::path() -> zpt::json {
    return this->__path;
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::content() -> T& {
    return this->__content;
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::set_path(std::string const& _path)
...
template<typename T>
auto zpt::pipeline::event<T>::event_t::set_content(T const& _content) -> zpt::pipeline::event<T>::event_t& {
    this->__content = _content;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::event_t::trigger(std::string const& _uri, T _content)
  -> zpt::pipeline::event<T>::event_t& {
...
template<typename T>
auto zpt::pipeline::event<T>::event_t::trigger(zpt::json _uri, T _content)
  -> zpt::pipeline::event<T>::event_t& {
    this->__parent->trigger(_uri, _content);
    return (*this);
}
...
template<typename T>
auto zpt::pipeline::event<T>::event_t::steps() -> zpt::pipeline::event<T>::event_t::position& {
...
template<typename T>
auto zpt::pipeline::event<T>::pause() -> zpt::pipeline::event<T>& {
    this->__underlying->__current_step.__pause = true;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::resume() -> zpt::pipeline::event<T>& {
...
template<typename T>
auto zpt::pipeline::event<T>::cancel() -> zpt::pipeline::event<T>& {
    this->__underlying->__current_step.__pause = true;
    return (*this);
}

template<typename T>
auto zpt::pipeline::event<T>::send_to_next_step() -> void {
...
template<typename T>
auto zpt::pipeline::step<T>::trapped(zpt::json _path, zpt::pipeline::event<T> _event) -> void {
...
template<typename T>
auto zpt::pipeline::step<T>::listen_to(zpt::json _path, event_callback _callback) -> void {
...
template<typename T>
auto zpt::pipeline::step<T>::report_error(zpt::json& _event,
                                          zpt::pipeline::event<T>& _content,
                                          const char* _what,
                                          const char* _description,
                                          const char* _backtrace,
...
template<typename T>
auto zpt::pipeline::step<T>::set_error_callback(error_callback _error_callback) -> zpt::pipeline::step<T>& {
...
zpt::pipeline::engine<T>::engine(size_t _pipeline_size, zpt::json _step_queue_configuration)
  ...
  , __threads_per_step{ std::max(static_cast<long>(_step_queue_configuration["max_queue_threads"]), 1L) } {
    long _max_threads = std::max(static_cast<long>(_step_queue_configuration["max_queue_threads"]), 1L) *
...
template<typename T>
auto zpt::pipeline::engine<T>::size() -> size_t {
    return this->__pipeline_size;
}

template<typename T>
auto zpt::pipeline::engine<T>::start_threads() -> zpt::pipeline::engine<T>& {
    size_t _n_step{ 1 };
...
template<typename T>
auto zpt::pipeline::engine<T>::shutdown() -> zpt::pipeline::engine<T>& {
    size_t _n_step{ 1 };
    for (auto _step : this->__steps) {
        _step->shutdown();
...
template<typename T>
auto zpt::pipeline::engine<T>::is_shutdown_ongoing() -> bool {
    for (auto _step : this->__steps) {
...
template<typename T>
auto zpt::pipeline::engine<T>::add_listener(size_t _step, std::string _pattern, event_callback _callback)
  -> zpt::pipeline::engine<T>& {
...
template<typename T>
auto zpt::pipeline::engine<T>::trigger(std::string const& _uri, T _content) -> zpt::pipeline::engine<T>& {
    return this->trigger(zpt::pipeline::to_path(_uri), _content);
}

template<typename T>
auto zpt::pipeline::engine<T>::trigger(zpt::json _uri, T _content) -> zpt::pipeline::engine<T>& {
    this->__steps[0]->trigger(_uri, zpt::pipeline::event<T>{ *this, _content, _uri });
    return (*this);
}

template<typename T>
auto zpt::pipeline::engine<T>::set_error_callback(error_callback _error_callback)
  -> zpt::pipeline::engine<T>& {
...
template<typename T>
auto zpt::pipeline::engine<T>::send_to_next_step(zpt::pipeline::event<T> _event)
  -> zpt::pipeline::engine<T>& {
