...
zpt::json _r_channels = _emitter->events()->route(zpt::ev::Get, zpt::path::join({ zpt::array, "v2", "datums", "channel-subscriptions" }), { "params", { "app_id", std::string(_r_data["id"]) } })["payload"];
_r_data << "channels" << (_r_channels["elements"]->type() == zpt::JSArray ? _r_channels["elements"] : zpt::json({ zpt::array, _r_channels }));
_c->insert("MyUsers", _envelope["payload"]["href"]->str(), _r_data, { "mutated-event", true });
}
}
}
...
_c->set("MyUsers", _envelope["payload"]["filter"], _envelope["payload"]["changes"], { "mutated-event", true });
}
else {
_c->set("MyUsers", _envelope["payload"]["href"]->str(), _envelope["payload"]["changes"], { "mutated-event", true });
}
}
}
}
...
zpt::connector _c = _emitter->connector(_c_name->str());
if (_envelope["payload"]["filter"]->ok()) {
_c->remove("MyUsers", _envelope["payload"]["filter"], { "mutated-event", true });
}
else {
_c->remove("MyUsers", _envelope["payload"]["href"]->str(), { "mutated-event", true });
}
}
}
}
...
zpt::json _r_channels = _emitter->events()->route(zpt::ev::Get, zpt::path::join({ zpt::array, "v2", "datums", "channel-subscriptions" }), { "params", { "app_id", std::string(_r_data["id"]) } })["payload"];
_r_data << "channels" << (_r_channels["elements"]->type() == zpt::JSArray ? _r_channels["elements"] : zpt::json({ zpt::array, _r_channels }));
_c->save("MyUsers", _envelope["payload"]["href"]->str(), _r_data, { "mutated-event", true });
}
}
}
...
zpt::json _r_targets = _c->query("MyUsers", { "id", std::string(_r_element["app_id"]) });
for (auto _r_data : _r_targets["elements"]->arr()) {
zpt::json _r_channels = _emitter->events()->route(zpt::ev::Get, zpt::path::join({ zpt::array, "v2", "datums", "channel-subscriptions" }), { "params", { "app_id", std::string(_r_data["id"]) } })["payload"];
_c->set("MyUsers", _r_data["href"]->str(), { "channels", (_r_channels["elements"]->type() == zpt::JSArray ? _r_channels["elements"] : zpt::json({ zpt::array, _r_channels })) }, { "href", _r_data["href"] });
}
}
}
})
...
for (auto _c_name : _c_names->arr()) {
zpt::connector _c = _emitter->connector(_c_name->str());
zpt::json _r_targets = _c->query("MyUsers", { "channels", { "href", _envelope["payload"]["href"] } });
for (auto _r_data : _r_targets["elements"]->arr()) {
zpt::json _r_channels = _emitter->events()->route(zpt::ev::Get, zpt::path::join({ zpt::array, "v2", "datums", "channel-subscriptions" }), { "params", { "app_id", std::string(_r_data["id"]) } })["payload"];
_c->set("MyUsers", _r_data["href"]->str(), { "channels", (_r_channels["elements"]->type() == zpt::JSArray ? _r_channels["elements"] : zpt::json({ zpt::array, _r_channels })) }, { "href", _r_data["href"] });
}
