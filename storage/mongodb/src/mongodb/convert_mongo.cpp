    ...
    for (auto _i : *_in) {
        ...
        switch (_value->type()) {
            case zpt::JSObject: {
                ...
                zpt::mongodb::tomongo(_value->object(), _mobj);
                _out << _key << _mobj.object();
                break;
            }
            case zpt::JSArray: {
                mongo::BSONArrayBuilder _mobj;
                zpt::mongodb::tomongo(_value->array(), _mobj);
                _out << _key << _mobj.array();
                break;
            }
            case zpt::JSString: {
...
void
zpt::mongodb::tosetcommand(zpt::JSONObj& _in, mongo::BSONObjBuilder& _out, std::string const& _prefix) {
    for (auto _i : *_in) {
        std::string _key = _i.first;
        JSONElement _value = _i.second;
        ...
        switch (_value->type()) {
            case zpt::JSObject: {
                zpt::mongodb::tosetcommand(
                  _value->object(), _out, (_prefix.length() != 0 ? _prefix + std::string(".") + _key : _key));
                break;
            }
            case zpt::JSArray: {
                zpt::mongodb::tosetcommand(
                  _value->array(), _out, (_prefix.length() != 0 ? _prefix + std::string(".") + _key : _key));
                break;
            }
            case zpt::JSString: {
...
void
zpt::mongodb::tosetcommand(zpt::JSONArr& _in, mongo::BSONObjBuilder& _out, std::string const& _prefix) {
    size_t _idx = 0;
    for (auto _i : *_in) {
        zpt::json _value = _i;
        ...
        switch (_value->type()) {
            case zpt::JSObject: {
                zpt::mongodb::tosetcommand(
                  _value->object(),
                  _out,
                  (_prefix.length() != 0 ? _prefix + std::string(".") + std::to_string(_idx) : std::to_string(_idx)));
                break;
            }
            case zpt::JSArray: {
                zpt::mongodb::tosetcommand(
...
zpt::mongodb::get_query(zpt::json _in, mongo::BSONObjBuilder& _queryr) {
    ...
    for (auto _i : _in->object()) {
        ...
        if (_value->type() == zpt::JSArray) {
            ...
            zpt::mongodb::tomongo(_value->array(), _tmp);
            _queryr.append(key, _tmp.array());
            continue;
        }
        ...
        if (value.length() > 3 && value.find('/') != std::string::npos) {
            ...
            while (std::getline(lss, part, '/')) {
                if (bar_count == 0) {
                    ...
                    if (expression.length() == 0 || expression[expression.length() - 1] != '\\') { ++bar_count; }
                    else {
                ...
                if (bar_count == 2) {
                    ...
                    if (!iss.eof()) {
                        ...
                        if (!iss.eof()) {
                            ...
                            if (bexpr != "true" && bexpr != "false") { _queryr.append(key, expression); }
                    ...
                    if (bar_count == 2) { _queryr.append(key, BSON(comp << expression)); }
                        ...
                        try {
                            ...
                            if (_json->type() == zpt::JSObject) {
                                ...
                                _queryr.append(key, BSON(comp << _mongo_json.object()));
                            }
                            else if (_json->type() == zpt::JSArray) {
                                mongo::BSONArrayBuilder _mongo_json;
                                zpt::mongodb::tomongo(_json, _mongo_json);
                                _queryr.append(key, BSON(comp << _mongo_json.array()));
                            }
                        }
                        ...
                        mongo::BSONObjBuilder _mongo_json;
                        zpt::mongodb::tomongo(_json, _mongo_json);
                        _queryr.append(key, BSON(comp << _mongo_json.object()["time"]));
                    }
                    continue;
                }
            }
