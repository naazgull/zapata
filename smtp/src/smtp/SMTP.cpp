...
auto zpt::SMTP::compose(zpt::json _e_mail) -> std::string {
	...
	for (auto _r : _e_mail["To"]->arr()) {
		...
		first = false;
		_recipients += (_parsed["name"]->ok() ? zpt::quoted_printable::r_encode(std::string(_parsed["name"]), "utf-8") + std::string(" <") : std::string("")) + std::string(_parsed["address"]) + (_parsed["name"]->ok() ? std::string(">") : std::string(""));
	}

	zpt::json _from = zpt::email::parse(std::string(_e_mail["From"]));
	_oss << "Date: " << zpt::tostr(time(nullptr), "%a, %d %b %Y %X %Z") << CRLF << flush;
	_oss << "From: " << (_from["name"]->ok() ? zpt::quoted_printable::r_encode(std::string(_from["name"]), "utf-8") + std::string(" <") : std::string("")) << std::string(_from["address"]) << (_from["name"]->ok() ? std::string(">") : std::string("")) << CRLF << flush;
	...
	_oss << "Subject: " << zpt::quoted_printable::r_encode(std::string(_e_mail["Subject"]), "utf-8") << CRLF << flush;
	if (_e_mail["Reply-To"]->ok()) {
		zpt::json _reply_to = zpt::email::parse(std::string(_e_mail["Reply-To"]));
		_oss << "Reply-To: " << (_reply_to["name"]->ok() ? zpt::quoted_printable::r_encode(std::string(_reply_to["name"]), "utf-8") + std::string(" <") : std::string("")) << std::string(_reply_to["address"]) << (_reply_to["name"]->ok() ? std::string(">") : std::string("")) << CRLF << flush;
	}
...
auto zpt::SMTP::open() -> mailsmtp* {
	...
	try {
		if(this->__type[1] == zpt::json::string("ssl")) {
			assertz(mailsmtp_ssl_connect(_smtp, this->__host.data(), this->__port) == MAILSMTP_NO_ERROR, std::string("could not connect to ") + this->__connection, 503, 1501);
		}
		else if(this->__type[1] == zpt::json::string("tls")) {	
			assertz(mailsmtp_socket_connect(_smtp, this->__host.data(), this->__port) == MAILSMTP_NO_ERROR, std::string("could not connect to ") + this->__connection, 503, 1501);
		}
		else {
			assertz(mailsmtp_socket_connect(_smtp, this->__host.data(), this->__port) == MAILSMTP_NO_ERROR, std::string("could not connect to ") + this->__connection, 503, 1501);
		}
		...
		if (this->__type[0] == zpt::json::string("smtp")) {
			assertz(mailsmtp_helo(_smtp) == MAILSMTP_NO_ERROR, std::string("could not introduce my self to ") + this->__connection + std::string(" using SMTP"), 400, 1502);
		}
		else if (this->__type[0] == zpt::json::string("esmtp")) {
			assertz(mailesmtp_ehlo(_smtp) == MAILSMTP_NO_ERROR, std::string("could not introduce my self to ") + this->__connection + std::string(" using ESMTP"), 400, 1502);
		}

		if (this->__type[0] == zpt::json::string("esmtp") && this->__type[1] == zpt::json::string("tls")) {
			assertz(mailsmtp_socket_starttls(_smtp) == MAILSMTP_NO_ERROR, std::string("could not STARTTLS with ") + this->__connection, 403, 1503);
			assertz(mailesmtp_ehlo(_smtp) == MAILSMTP_NO_ERROR, std::string("could not introduce my self to ") + this->__connection + std::string(" using ESMTP"), 400, 1502);
		}

		if (this->__user.length() != 0 && this->__type[0] == zpt::json::string("esmtp")) {
			assertz(mailsmtp_auth(_smtp, this->__user.data(), this->__passwd.data()) == MAILSMTP_NO_ERROR, std::string("could not authenticate propertly with ") + this->__connection, 401, 1504);
		}
	}
...
auto zpt::SMTP::send(zpt::json _e_mail) -> void {
	...
	try {
		if (this->__type[0] == zpt::json::string("esmtp")) {
			assertz(mailesmtp_mail(_smtp, std::string(_e_mail["From"]["address"]).data(), 1, "etPanSMTPZapataWrapper") == MAILSMTP_NO_ERROR, std::string("could not send email through ") + this->__connection, 502, 1505);
		}
		else {
			assertz(mailsmtp_mail(_smtp, std::string(_e_mail["From"]["address"]).data()) == MAILSMTP_NO_ERROR, std::string("could not send email through ") + this->__connection, 502, 1505);
		}
		...
		for (auto _r : _e_mail["To"]->arr()) {
			if (this->__type[0] == zpt::json::string("esmtp")) {
				assertz(mailesmtp_rcpt(_smtp, std::string(_r["address"]).data(), MAILSMTP_DSN_NOTIFY_FAILURE | MAILSMTP_DSN_NOTIFY_DELAY, nullptr) == MAILSMTP_NO_ERROR, std::string("could not add recipient ") + std::string(_r["address"]), 400, 1507);
			}
			else {
				assertz(mailsmtp_rcpt(_smtp, std::string(_r["address"]).data()) == MAILSMTP_NO_ERROR, std::string("could not add recipient ") + std::string(_r["address"]), 400, 1507);
			}
		}

		assertz(mailsmtp_data(_smtp) == MAILSMTP_NO_ERROR, std::string("could not send DATA command to ") + this->__connection, 502, 1508);
		assertz(mailsmtp_data_message(_smtp, _mimed_mail.data(), _mimed_mail.length()) == MAILSMTP_NO_ERROR, std::string("could not send email through ") + this->__connection, 502, 1509);
