...
class filter {
  public:
    ...
    template<typename T, typename... Args>
    static auto alloc(Args... _args) -> zpt::storage::filter;

  private:
    std::shared_ptr<zpt::storage::filter::type> __underlying{ nullptr };

    filter(zpt::storage::filter::type* _underlying);
};
class result {
  public:
    class type {
...
template<typename T, typename... Args>
auto
zpt::storage::filter::alloc(Args... _args) -> zpt::storage::filter {
    return zpt::storage::filter{ new T{ _args... } };
}
