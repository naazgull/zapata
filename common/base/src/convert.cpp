...
auto zpt::tostr(std::string& s, float i, int precision) -> void {
    std::ostringstream _oss;
    if (precision != 0) { _oss << std::setprecision(precision); }
    _oss << std::fixed << std::noshowpoint << i << std::flush;
    s.insert(s.length(), _oss.str());
}
...
auto zpt::fromstr(std::string_view s, bool* i) -> void { *i = s == std::string_view("true"); }
...
auto zpt::timestamp_to_str(std::uint64_t _timestamp) -> std::string {
    std::chrono::sys_time<std::chrono::milliseconds> _milli{ std::chrono::milliseconds{
      _timestamp } };
    return std::format("{:%Y-%m-%dT%H:%M:%S}", _milli);
}
