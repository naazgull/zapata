...
auto zpt::mysql::LogEventPtr::register_callback(
...
auto zpt::mysql::LogEventHeader::to_json() -> zpt::json {
	return {"timestamp",
		...
		"type_code",
		(unsigned int)this->__type_code,
		"server_id",
		(unsigned long)this->__server_id,
		"event_length",
		(unsigned long)this->__event_length,
		"next_position",
		(unsigned long)this->__next_position,
		"flags",
		this->__flags.to_string()};
}

auto zpt::mysql::LogEvent::header() -> zpt::mysql::event_header { return this->__header; }

auto zpt::mysql::LogEvent::header(zpt::mysql::event_header _header) -> void { this->__header.swap(_header); }
...
auto zpt::mysql::QueryEvent::consume(zpt::mysql::event_header _header, std::istream& _in) -> zpt::mysql::event {
	...
	if (_header->__event_length) {
		...
		for (; _idx != _event->__variable_length;) {
			...
			switch (_var_type) {
			case zpt::mysql::Q_FLAGS2_CODE: { // Value is a 4-byte bit-field. This variable is written only
							  // as of MySQL 5.0.
			...
			case zpt::mysql::Q_CATALOG_CODE: { // Value is the catalog name: a length byte followed by that
			...
			case zpt::mysql::Q_AUTO_INCREMENT: { // Value is two 2-byte unsigned integers representing the
			...
			case zpt::mysql::Q_CHARSET_CODE: { // Value is three 2-byte unsigned integers representing the
			...
			case zpt::mysql::Q_TIME_ZONE_CODE: { // Value is the time zone name: a length byte followed by
			...
			case zpt::mysql::Q_CATALOG_NZ_CODE: { // Value is the catalog name: a length byte followed by
			...
			case zpt::mysql::Q_LC_TIME_NAMES_CODE: { // Value is a 2-byte unsigned integer representing the
			...
			case zpt::mysql::Q_CHARSET_DATABASE_CODE: { // Value is a 2-byte unsigned integer representing
			...
			case zpt::mysql::Q_TABLE_MAP_FOR_UPDATE_CODE: { // Value is 8 bytes representing the table map
			...
			case zpt::mysql::Q_INVOKER: {
				new (&_q_var.__invoker) std::tuple<std::string, std::string>;
				{
					...
					char _var_value[_var_length] = {0};
					_in.read(_var_value, _var_length);
					std::get<0>(_q_var.__invoker) = zpt::mysql::arr2str(_var_value, _var_length);
					_idx += sizeof(uint8_t) + _var_length;
				}
				{
					uint8_t _var_length = 0;
					...
					char _var_value[_var_length] = {0};
					_in.read(_var_value, _var_length);
					std::get<1>(_q_var.__invoker) = zpt::mysql::arr2str(_var_value, _var_length);
					_idx += sizeof(uint8_t) + _var_length;
				}
				break;
			}
			case zpt::mysql::Q_UPDATED_DB_NAMES: {
				...
				for (uint8_t _arr_idx = 0; _arr_idx != _arr_length; _arr_idx++) {
					...
					_idx += _str_len;
					_q_var.__updated_db_names.push_back(zpt::mysql::arr2str(_var_value, _str_len));
				}
				break;
			}
		...
		size_t _query_length = _event->header()->__event_length - zpt::mysql::lengths::header -
				       zpt::mysql::lengths::fixed::query_event - _event->__database_name_length -
				       _event->__variable_length;
...
auto zpt::mysql::FormatDescriptionEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::BeginLoadQueryEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::ExecuteLoadQueryEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::PreGAWriteRowsEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::PreGAUpdateRowsEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::PreGADeleteRowsEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::AnonymousGTIDLogEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
...
auto zpt::mysql::PreviousGTIDSLogEvent::consume(zpt::mysql::event_header _header, std::istream& _in)
    -> zpt::mysql::event {
