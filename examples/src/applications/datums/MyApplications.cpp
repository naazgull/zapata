...
if (_document["admin"]->ok()) _r_parent << "admin" << _document["admin"];
if (_document["app_secret"]->ok()) _r_parent << "app_secret" << _document["app_secret"];
if (_document["cloud_secret"]->ok()) _r_parent << "cloud_secret" << _document["cloud_secret"];
if (_document["device_secret"]->ok()) _r_parent << "device_secret" << _document["device_secret"];
if (_document["icon"]->ok()) _r_parent << "icon" << _document["icon"];
if (_document["image"]->ok()) _r_parent << "image" << _document["image"];
if (_document["name"]->ok()) _r_parent << "name" << _document["name"];
if (_document["namespace"]->ok()) _r_parent << "namespace" << _document["namespace"];
if (_document["token"]->ok()) _r_parent << "token" << _document["token"];
if (_document["users"]->ok()) _r_parent << "users" << _document["users"];
...
if (_document["token"]->ok()) _r_parent << "token" << _document["token"];
if (_document["users"]->ok()) _r_parent << "users" << _document["users"];
_r_data = { "href", _topic, "n_updated", _c->save("MyApplications", _topic, _document - _r_parent, { "href", _topic }) };
...
if (_document["token"]->ok()) _r_parent << "token" << _document["token"];
if (_document["users"]->ok()) _r_parent << "users" << _document["users"];
_r_data = { "href", _topic, "n_updated", _c->set("MyApplications", _topic, _document - _r_parent, { "href", _topic }) };
...
_document <<
"updated" << zpt::json::date();
zpt::json _r_elements = zpt::apps::datums::MyApplications::query(_topic, _filter, _emitter, _identity, _envelope);
for (auto _r_element : _r_elements["elements"]->arr()) {
zpt::apps::datums::MyApplications::set(_r_element["href"]->str(), _document, _emitter, _identity, _envelope);
}
